from __future__ import annotations

from src.core.errors import DeviceProcessError, TransportError
from src.core.logging_utils import EventLogger
from src.core.models import CommandResult, DeviceHealth, DeviceName, ResultCode
from src.devices.base_worker import BaseWorker
from src.transport.legacy_adapters import LegacySpectrometerAdapter


class SpectrometerWorker(BaseWorker):
    def __init__(self, adapter: LegacySpectrometerAdapter, logger: EventLogger) -> None:
        super().__init__(DeviceName.SPECTROMETER, logger)
        self.adapter = adapter

    def open(self) -> None:
        try:
            self.adapter.open()
            self._set_health(DeviceHealth.HEALTHY, "opened")
        except Exception as exc:
            self._set_health(DeviceHealth.DISCONNECTED, str(exc))
            raise

    def close(self) -> None:
        self.adapter.close()
        self._set_health(DeviceHealth.DISCONNECTED, "closed")

    def healthcheck(self) -> None:
        try:
            self.adapter.healthcheck()
            self._set_health(DeviceHealth.HEALTHY, "healthcheck ok")
        except Exception as exc:
            self._set_health(DeviceHealth.UNHEALTHY, str(exc))
            raise

    def execute_acquire(self) -> CommandResult:
        cmd = "SPECTRUM GET"
        self._begin_command(cmd)
        try:
            spectrum = self.adapter.get_spectrum()
            self._set_health(DeviceHealth.HEALTHY, "acquire ok")
            return self._finish_command(
                CommandResult(
                    device=self.name,
                    command=cmd,
                    code=ResultCode.OK,
                    detail="spectrum acquired",
                    payload=spectrum,
                )
            )
        except (TransportError, DeviceProcessError) as exc:
            self._set_health(DeviceHealth.UNHEALTHY, str(exc))
            return self._finish_command(
                CommandResult(
                    device=self.name,
                    command=cmd,
                    code=ResultCode.ERROR,
                    detail=str(exc),
                )
            )