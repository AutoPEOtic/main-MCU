...
class sender():
    ...
    def send_instruction(self, instruction, line):
        ...
        if instruction=="" or instruction.startswith('#'):  pass
            ...
            if instruction.startswith('PUMP1'): self.peripherals.send_instruction('a' + delay)
            elif instruction.startswith('PUMP2'): self.peripherals.send_instruction('b' + delay)
        ...
        elif instruction.startswith('FAN'): _, delay = instruction.split(' '); self.peripherals.send_instruction('d' + delay)
        elif instruction.startswith('WIRE CUT'):    self.peripherals.send_instruction('g000')
        elif instruction.startswith('SPECTRUM GET'):    self.autopeotic_db.send(self.autopeotic_db.generate_query(self.spectrum.get_spectrum()))
        elif instruction.startswith(('G1', 'G21', 'G90', 'G91', 'M30', 'F')):   self.stepper.send_instruction(instruction)
        elif instruction.startswith('SEND PEO VALUES'): self.peo.send_values()
        elif instruction.startswith('PEO ON'):  self.peo.on()
